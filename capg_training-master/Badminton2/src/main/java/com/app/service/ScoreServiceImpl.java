...
public class ScoreServiceImpl implements ScoreService {
	...
	public Players fetchtheWinner() {
		...
		Players winner = playerRepository.findById(sc.getWinner()).get(); // we have to use .get() to avoid writing Optional<Players> in the LHS
		System.out.println(sc);
	...
	public Players fetchtheRunnerUp() {
		...
		if (sc.getWinner()!=sc.getPlayerid1())
			runnerup = playerRepository.findById(sc.getPlayerid1()).get(); // we have to use .get() to avoid writing Optional<Players> in the LHS
		else 
